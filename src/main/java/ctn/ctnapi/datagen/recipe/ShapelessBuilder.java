...
public class ShapelessBuilder {
	...
	public ShapelessBuilder basicUnlockedBy() {
		try {
			...
			// 以结果堆栈的描述ID和物品设置解锁条件
			Criterion<InventoryChangeTrigger.TriggerInstance> result =
					(Criterion) itemLikeMethod.invoke(null, resultStack.getItem());
			...
			for (Ingredient ingredient : ingredients) {
				...
				for (Ingredient.Value value : ingredientTag) {
					if (value instanceof Ingredient.TagValue(TagKey<Item> tag)) {
						...
						Criterion<InventoryChangeTrigger.TriggerInstance> has =
								(Criterion) tagMethod.invoke(null, tag);
					...
					if (value instanceof Ingredient.ItemValue(ItemStack stack)) {
						...
						Criterion<InventoryChangeTrigger.TriggerInstance> has =
								(Criterion) itemLikeMethod.invoke(null, item);
						this.unlockedBy(stackName, has);
					}
				}
			}
		} catch (Exception e) {
			CtnApiMain.LOGGER.debug(e);
			return this;
		}
		return this;
	}
